...
function NavMenuItem({ item, pathname }: { item: MenuItem; pathname: string }) {
	return (
...
function DashboardLayoutContent({
	...
	const gatherSystemNavGroups = useGatherSystemShell
		? getAuthorizedGatherSystemNavGroups(
				userRole,
				user?.assignedMinistryIds ?? [],
				user?.is_active ?? true
				...
				<div className="flex flex-1">
					<Sidebar>
						<SidebarContent>
							...
							<SidebarMenu>
								{useGatherSystemShell
									? gatherSystemNavGroups.map((group) => (
