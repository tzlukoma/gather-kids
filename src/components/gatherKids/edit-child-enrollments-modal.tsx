		...
		try {
			...
			for (const ministry of ministries) {
				...
				const existingEnrollment = currentEnrollments.find(
					(e) =>
						e.ministry_id === ministry.ministry_id &&
						e.cycle_id === currentCycle
				);
				...
				if (isEnrolled && !existingEnrollment) {
					...
					const currentInterest =
						existingEnrollment.custom_fields?.interest || false;
	...
	return (
		<Dialog open={true} onOpenChange={onClose}>
			<DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
				<DialogHeader>
					<DialogTitle>
						Edit {child.first_name}'s Ministry Enrollments
					</DialogTitle>
					<DialogDescription>
						Manage {child.first_name}'s ministry enrollments for the current
						year.
					</DialogDescription>
				</DialogHeader>
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
						{ministries.map((ministry) => {
							const isEnrolled =
								form.watch(`ministrySelections.${ministry.ministry_id}`) ||
								false;
							const isInterested =
								form.watch(`interestSelections.${ministry.ministry_id}`) ||
								false;
								...
								<div
									...
									<div className="flex items-center justify-between">
										...
										<div className="text-sm text-muted-foreground">
											Ages {ministry.min_age}-{ministry.max_age} | Grades{' '}
