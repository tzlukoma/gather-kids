...
interface CheckoutDialogProps {
  ...
  onClose: () => void
  onCheckout: (childId: string, attendanceId: string, verifier: { method: 'PIN' | 'other', value: string }) => void
}
  ...
  const handleVerifyAndCheckout = () => {
    ...
    if (validPins.includes(pin)) {
      if (child.activeAttendance?.attendance_id) {
        onCheckout(child.child_id, child.activeAttendance.attendance_id, { method: 'PIN', value: pin })
      }
      handleClose()
    } else {
      toast({
  ...
  const handleOverrideCheckout = () => {
    ...
    if (!overrideName.trim()) {
        toast({
            variant: "destructive",
            title: "Name required",
            description: "Please enter the name of the person picking up the child.",
        });
        return;
    }
     if (child.activeAttendance?.attendance_id) {
        onCheckout(child.child_id, child.activeAttendance.attendance_id, { method: 'other', value: overrideName.trim() })
    }
    handleClose();
  }
    ...
    <Dialog open={!!child} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent>
        ...
        <DialogFooter className="sm:justify-between">
            {mode === 'pin' ? (
                <Button variant="outline" onClick={() => setMode('override')}>Admin Override</Button>
            ) : (
                <Button variant="outline" onClick={() => setMode('pin')}>Back to PIN</Button>
            )}
