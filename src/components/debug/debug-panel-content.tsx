...
export function DebugPanelContent() {
	...
	const calculateDataSources = useCallback(
		(eventsToAnalyze: AnyDebugEvent[]) => {
			const sources = [
				{ name: 'dbAdapter', active: false, count: 0 },
				{ name: 'IndexedDB', active: false, count: 0 },
				{ name: 'Direct DB', active: false, count: 0 },
	...
	const loadEventsFromStore = useCallback(() => {
		...
		console.log(
			'🔍 Debug Panel: Loading events from global store, count:',
			storedEvents.length
		);
		...
		const currentRouteEvents = storedEvents.filter(
			(event) => event.route === currentRoute
		);
		...
		console.log(
			'🔍 Debug Panel: Updated data sources - All:',
			newAllDataSources,
			'Route:',
			newRouteDataSources
		);
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription>
						Current environment configuration and database connection details
					</CardDescription>
				</CardHeader>
							...
							<Badge
								variant={
									key === 'NODE_ENV' && value === 'production'
										? 'destructive'
										: key === 'SUPABASE_URL' &&
										  value?.includes('tfvzdjaebqirmowvgowz')
										? 'default'
										: 'secondary'
								}
								className={
								...
								{key === 'SUPABASE_URL' && value
									? `${value.split('//')[1]?.split('.')[0]}...` // Show project ID only
									: String(value)}
							</Badge>
						</div>
					))}
					{environmentInfo.SUPABASE_URL?.includes('tfvzdjaebqirmowvgowz') && (
						<div className="text-xs text-green-600 bg-green-50 p-2 rounded border border-green-200">
							✅ Connected to <strong>PRODUCTION</strong> Supabase database
						</div>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				...
				<CardContent>{renderOperations(routeEvents, 'max-h-48')}</CardContent>
			</Card>
			...
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription>Data sources used across all routes</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="flex flex-wrap gap-2">
			...
			<Card>
				...
				<CardContent>
					{events.length === 0 ? (
						<div className="text-center text-muted-foreground py-4">
							No operations recorded yet. Navigate around the app to see debug
							events.
						</div>
					) : (
				...
				<CardContent className="space-y-3">
					<div className="flex flex-wrap gap-2">
						...
						<Button
							...
							variant="outline"
							onClick={() =>
								console.log('Debug Events:', { routeEvents, allEvents: events })
							}>
							Log to Console
						</Button>
					</div>
					...
					<div className="text-xs text-muted-foreground space-y-1">
						<p>
							<strong>Toggle:</strong> Set <code>{DEBUG_LS_KEY}='1'</code> in
							localStorage
						</p>
						<p>
						...
						<p>
							<strong>Privacy:</strong> Only method names and paths are logged
							(no payloads)
						</p>
					</div>
				</CardContent>
