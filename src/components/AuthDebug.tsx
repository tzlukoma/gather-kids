            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="bg-muted p-2 rounded text-xs">
                  ...
                  <p><strong>Created:</strong> {user?.created_at ? new Date(user.created_at).toLocaleString() : 'None'}</p>
                </div>
              </div>
                  ...
                  <div className="bg-muted p-2 rounded text-xs">
                    {user.identities.map((identity: any, index: number) => (
                      <div key={index} className="mb-2 last:mb-0">
                        ...
                        <p><strong>Identity ID:</strong> {identity.id}</p>
                        <p><strong>Created:</strong> {new Date(identity.created_at).toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {!user && (
                <div>
                  <h4 className="text-sm font-medium mb-2">No Authentication</h4>
                  <div className="bg-muted p-2 rounded text-xs">
