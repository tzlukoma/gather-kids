	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (formData.use_logo_only && !formData.logo_url) {
			toast({
				title: 'Logo Required',
				description:
					'When "Use Logo Only" is enabled, a logo file must be uploaded.',
				variant: 'destructive',
			});
			return;
		}
				...
				<Card>
					...
					<CardContent className="space-y-4">
						...
						<div>
							...
							/>
							<p className="text-sm text-muted-foreground mt-1">
								This description appears on the home page and in marketing
								materials.
							</p>
						</div>
					</CardContent>
					...
					<CardContent className="space-y-4">
						...
						<div className="space-y-3">
							...
							<p className="text-sm text-muted-foreground">
								When enabled, only the logo will be displayed in headers (no app
								name text). A logo file is required when this option is
								selected.
							</p>
							{formData.use_logo_only && !formData.logo_url && (
								<p className="text-sm text-destructive font-medium">
									⚠️ A logo file is required when &quot;Use Logo Only&quot; is
									enabled.
								</p>
							)}
						</div>
