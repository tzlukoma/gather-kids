...
export default function BibleBeeAdminPage() {
  ...
  async function handleCreate() {
    const y = await createCompetitionYear({ year: new Date().getFullYear(), name: `Year ${new Date().getFullYear()}` });
        ...
        {years.map(y => (
          <Card key={y.id}>
            ...
            <CardContent>
              ...
              <div className="text-sm text-muted-foreground">{y.opensAt ? `Opens: ${y.opensAt}` : ''}</div>
            </CardContent>
            <CardFooter>
              <Link href={`/dashboard/bible-bee/year/${y.id}`} className="text-primary underline">Manage</Link>
            </CardFooter>
          </Card>
        ))}
      </div>
