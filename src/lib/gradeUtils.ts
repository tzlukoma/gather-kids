...
/** Store grades as canonical numeric strings (-1|0|1..12) for DB consistency. */
export function canonicalizeGradeForStorage(gradeText?: string): string {
    if (gradeText === null || gradeText === undefined) return '';
    const trimmed = String(gradeText).trim();
    if (!trimmed) return '';
    const code = gradeToCode(trimmed);
    if (code === null) return trimmed;
    return String(code);
}

/** Suggested grade for returning registration: bump by one year, cap at 12; Pre-K stays. */
export function suggestNextGradeCode(currentCode: number | null): number | null {
    if (currentCode === null) return null;
    if (currentCode === -1) return -1;
    if (currentCode >= 12) return 12;
