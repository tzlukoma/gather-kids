...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'divisions') AND
    ...
    RAISE NOTICE 'Updated divisions table foreign key constraints (new schema)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'essay_prompts') AND
    ...
    RAISE NOTICE 'Updated essay_prompts table foreign key constraints (new schema)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'enrollment_overrides') AND
    ...
    RAISE NOTICE 'Updated enrollment_overrides table foreign key constraints (new schema)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'scriptures') AND
    ...
    RAISE NOTICE 'Updated scriptures table foreign key constraints (new schema)';
  END IF;
END $$;
