...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'scriptures') AND
    ...
    IF competition_years_id_type IS NOT NULL THEN
      ...
      RAISE NOTICE 'Updated scriptures table foreign key constraints (new schema) - competition_years.id is %', competition_years_id_type;
    ELSE
      RAISE NOTICE 'Skipped scriptures foreign key constraint - competition_years.id type unknown';
    END IF;
  END IF;
END $$;
