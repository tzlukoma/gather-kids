...
// This is a contract test suite that can be run against any database adapter
// Once Supabase adapter is implemented, this test should pass for both adapters
// If IndexedDB is not available in the test environment (Jest node), skip
// the full adapter contract tests which exercise Dexie. They should run in
// a browser-like environment or CI that supports IndexedDB.
const hasIndexedDB = typeof indexedDB !== 'undefined';

describe('Database Adapter Contract Tests', () => {
    if (!hasIndexedDB) {
        // eslint-disable-next-line jest/no-focused-tests
        it('skipped adapter contract tests: IndexedDB not available in test env', () => {
    ...
    beforeAll(async () => {
        // Clear test data in case previous tests failed
        try {
