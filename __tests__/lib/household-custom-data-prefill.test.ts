...
describe.each([
	{
		...
		selectionKey: 'ministrySelections' as const,
		enrollmentType: 'enrolled' as const,
	},
	{
		status: 'expressed_interest' as const,
		selectionKey: 'interestSelections' as const,
		enrollmentType: 'expressed_interest' as const,
	},
])(
	'fetchFullHouseholdDataFromAdapter customData prefill ($status)',
	({ status, selectionKey, enrollmentType }) => {
		beforeEach(() => {
			jest.clearAllMocks();
		...
		it('maps enrollment custom_fields into child customData for wizard controls', async () => {
			const profile = await fetchFullHouseholdDataFromAdapter(
				'household-1',
				'cycle-1'
			);
			...
			expect(profile.children[0][selectionKey]).toEqual({
