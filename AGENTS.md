...
**Stack (as shipped):** Next.js 16, React 19, TypeScript, Supabase Auth + Postgres, TanStack Query, React Hook Form + Zod. Hosted on Vercel. UAT and production are separate Supabase projects.

Runtime data access is Supabase through the DAL (`src/lib/database/`). Demo/IndexedDB mode is **not** a supported runtime. Do not revive Dexie or treat leftover historical docs as the current architecture.
