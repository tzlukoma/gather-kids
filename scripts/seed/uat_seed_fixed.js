...
async function createScriptures(yearId) {
	...
	for (let i = 1; i <= scCount; i++) {
		...
		if (existing) {
			...
			if (insertError) {
				throw new Error(
					`Failed to create scripture ${scriptureData.title}: ${insertError.message}`
				);
			}
		...
		for (const translation of translations) {
			...
			if (checkTextError && checkTextError.code !== 'PGRST116') {
				throw new Error(
					`Error checking scripture text for ${textData.translation_abbreviation}: ${checkTextError.message}`
				);
			}

			if (existingText) {
				console.log(
					`✅ Scripture text already exists: ${textData.translation_abbreviation}`
				);
			} else {
				...
				if (insertTextError) {
					throw new Error(
						`Failed to create scripture text for ${textData.translation_abbreviation}: ${insertTextError.message}`
					);
				}

				console.log(
					`✅ Created scripture text: ${textData.translation_abbreviation}`
				);
			}
		}
	}
...
async function createEssayPrompt() {
	console.log(
		'⚠️  Skipping essay prompt creation - essay_prompts table not implemented yet'
	);
	return;
}
...
async function createMinistries() {
	...
	for (const ministryData of ministriesData) {
		...
		if (checkError && checkError.code !== 'PGRST116') {
			throw new Error(
				`Error checking ministry ${ministryData.name}: ${checkError.message}`
			);
		}
		...
		if (existing) {
			...
			if (insertError) {
				throw new Error(
					`Failed to create ministry ${ministryData.name}: ${insertError.message}`
				);
			}
...
async function createMinistryLeaders() {
	console.log(
		'⚠️  Skipping ministry leader assignment - ministry_accounts table not implemented yet'
	);
	return;
}
...
async function createMinistryEnrollments() {
	...
	for (const enrollmentData of enrollments) {
		if (!enrollmentData.child_id || !enrollmentData.ministry_id) {
			console.warn(
				`⚠️  Skipping enrollment ${enrollmentData.external_id}: missing child or ministry`
			);
			continue;
		}
		...
		if (checkError && checkError.code !== 'PGRST116') {
			throw new Error(
				`Error checking enrollment ${enrollmentData.external_id}: ${checkError.message}`
			);
		}

		if (existing) {
			console.log(
				`✅ Enrollment already exists: ${enrollmentData.external_id}`
			);
		} else {
			const { error: insertError } = await supabase
				.from('ministry_enrollments')
			...
			if (insertError) {
				throw new Error(
					`Failed to create enrollment ${enrollmentData.external_id}: ${insertError.message}`
				);
			}
...
async function seedUATData() {
	try {
		...
		console.log('📊 Summary:');
		console.log(
			'- 3 ministries (ministry leaders disabled - table not implemented)'
		);
		console.log(
			'- Competition year 2025-2026 (divisions disabled - table not implemented)'
		);
