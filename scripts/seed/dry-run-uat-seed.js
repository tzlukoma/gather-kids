...
const mockSingle = jest
	.fn()
	.mockReturnValue({ data: null, error: { code: 'PGRST116' } });
...
const mockData = {
	...
	competition_years: [{ id: 'mock-year-id', year: '2025-2026' }],
	divisions: [{ id: 'mock-division-id', name: 'Primary' }],
...
mockInsert.mockImplementation(function (data) {
	...
	if (data.id) {
		...
		return {
			select: () => ({
				single: () => ({
					data: { household_id: 'mock-household-id' },
					error: null,
				}),
			}),
		};
...
async function runDryRun() {
	...
	try {
		...
		console.log(
			'This does NOT guarantee the seed will work - it only verifies the script syntax.'
		);
		console.log(
			'Check the logs above for any issues with data formats or table structure.'
		);
	} catch (error) {
