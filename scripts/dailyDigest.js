...
async function verifyCheckpointTable() {
	...
	if (error) {
		console.error('Checkpoint table not found. Please run database migrations first.');
...
async function getLastCheckpoint() {
	...
	console.log(`No checkpoint found, using 24h ago: ${defaultCheckpoint.toISOString()}`);
...
function generateMinistryEmailContent(ministry, enrollments) {
	...
	let html = `
		...
		<p>You have received ${enrollments.length} new enrollment${enrollments.length !== 1 ? 's' : ''} since yesterday:</p>
		<ul>
	`;
	
	let text = `Daily Enrollment Digest - ${ministry.name}\n\n`;
	text += `You have received ${enrollments.length} new enrollment${enrollments.length !== 1 ? 's' : ''} since yesterday:\n\n`;
	...
	text += 'This is an automated daily digest from gatherKids. If you no longer wish to receive these emails, please contact your system administrator.';
...
function generateAdminEmailContent(enrollmentsByMinistry, totalCount) {
	...
	for (const [ministryId, group] of Object.entries(enrollmentsByMinistry)) {
		...
		html += `
			<h3>${ministry.name} (${enrollments.length} enrollment${enrollments.length !== 1 ? 's' : ''})</h3>
			<ul>
		`;
		
		text += `${ministry.name} (${enrollments.length} enrollment${enrollments.length !== 1 ? 's' : ''}):\n`;
...
async function sendDigestEmails(enrollmentsByMinistry, adminUsers) {
	...
	for (const [ministryId, group] of Object.entries(enrollmentsByMinistry)) {
		...
		if (!ministry.email) {
			console.log(`Skipping ministry ${ministry.name} - no contact email configured`);
			continue;
		}
	...
	if (adminUsers.length > 0) {
		const totalEnrollments = Object.values(enrollmentsByMinistry)
			.reduce((sum, group) => sum + group.enrollments.length, 0);
		
		const emailContent = generateAdminEmailContent(enrollmentsByMinistry, totalEnrollments);
...
async function main() {
	...
	try {
		...
		console.log(`Enrollments grouped into ${Object.keys(enrollmentsByMinistry).length} ministries`);
		...
		console.log(`- Ministry emails: ${results.ministry.sent} sent, ${results.ministry.failed} failed`);
		console.log(`- Admin emails: ${results.admin.sent} sent, ${results.admin.failed} failed`);
		
		// Update checkpoint only if all emails succeeded
		const totalFailed = results.ministry.failed + results.admin.failed;
		if (totalFailed === 0) {
			await updateCheckpoint(new Date());
			console.log('Daily digest completed successfully');
		} else {
			console.error(`${totalFailed} emails failed. Checkpoint not updated - will retry on next run.`);
			process.exit(1);
		}
