...
try {
	...
	if (adapter.constructor.name === 'SupabaseAdapter') {
		...
		console.log(
			'❌ Error: Using IndexedDB Adapter instead of Supabase Adapter'
		);
		console.log(
			'This indicates the feature flag logic is not working as expected'
		);
	}
} catch (error) {
...
console.log(
	'4. Check browser console for any database adapter initialization messages'
);
